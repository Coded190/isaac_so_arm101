...
def step_leader_followers(leader, *, so101=None, pingti=None) -> LeaderHwStep:
    ...
    # Same mapping as Kit: hardware tracks signed sim joints, not raw leader ±100.
...
def send_sim_joints_to_pingti(robot: Any, pingti) -> dict[str, float]:
    ...
    wrote = pingti.send_action(action)
    # Log the values actually handed to Feetech (slewed), not the far sim target.
    return wrote if isinstance(wrote, dict) and wrote else action
