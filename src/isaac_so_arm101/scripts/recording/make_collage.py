#!/usr/bin/env python3
r"""
make_collage.py — Build a 10x10 video collage with a cinematic zoom-out reveal.

Starts zoomed into one "hero" cell (filling the screen), then pulls back to
reveal the full 10x10 grid of random clips, like the TartanAir V2 trailer.

Windows version with hardcoded ffmpeg path.
Usage:
    python make_collage.py path\to\input.mp4 path\to\output.mp4
"""
...
# ─── FFMPEG PATH (Windows, hardcoded) ──────────────────────────────────────────
FFMPEG_DIR = r"C:\Users\saito\AppData\Local\Microsoft\WinGet\Packages\Gyan.FFmpeg_Microsoft.Winget.Source_8wekyb3d8bbwe\ffmpeg-8.1-full_build\bin"
...
def render_zoom(master_path: Path, hero_full_path: Path, output_path: str):
    ...
    # Filter graph:
    #   [0:v] master  -> zoompan  -> [zoomed]  (1920x1080, the zoom-out animation)
    #   [1:v] hero    -> add alpha + fade-out  -> [hero_fading]
    #   overlay [hero_fading] on top of [zoomed]
    zoompan_filter = (f"zoompan="
                      f"z='{z}':x='{x_expr}':y='{y_expr}':"
                      f"d=1:s={OUT_W}x{OUT_H}:fps={OUT_FPS}")

    # Hero stays fully opaque until HOLD_HERO, then fades out over HERO_FADE seconds.
...
def main():
    ...
    ap.add_argument("output", help="Path for output collage (e.g., collage.mp4).")
    args = ap.parse_args()
    ...
    if HOLD_HERO + ZOOM_DURATION >= CLIP_LEN:
        sys.exit(f"HOLD_HERO ({HOLD_HERO}) + ZOOM_DURATION ({ZOOM_DURATION}) "
                 f"must be less than CLIP_LEN ({CLIP_LEN}).")
