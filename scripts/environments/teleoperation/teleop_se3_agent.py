...
parser.add_argument(
    ...
    default=None,
    help=(
        "Orbit angle (degrees, clockwise from default position) around the palm tree crown centroid. "
...
app_launcher_args["enable_cameras"] = True  # required for TiledCamera sensors
...
def _leaf_world_positions(stage, palm_prim_path: str) -> list:
    """Return world-space XYZ translations of all leaf prims under the palm prim."""
    ...
    positions = []
    for prim in palm_prim.GetChildren():
        name = prim.GetName().lower()
        if "leaf" in name or "frond" in name or "palm" in name:
            xformable = UsdGeom.Xformable(prim)
            if xformable:
                world_xform = xformable.ComputeLocalToWorldTransform(0)
                tx = world_xform.GetRow3(3)
                positions.append((tx[0], tx[1], tx[2]))
    return positions


def get_crown_centroid(stage, palm_prim_path: str = "/World/envs/env_0/Scene/Palm"):
    """Compute XY centroid of palm crown (leaf) prims to account for trunk slant."""
    import numpy as np

    positions = _leaf_world_positions(stage, palm_prim_path)
    if not positions:
        # Fall back to base position (0, 0) if no leaf prims found
        return (0.0, 0.0)
...
def _yaw_to_quat_wxyz(yaw_rad: float) -> tuple:
    """Convert a Z-axis yaw (radians) to quaternion (w, x, y, z)."""
    import math
...
def reposition_robot_at_yaw(env, yaw_deg: float, palm_prim_path: str = "/World/envs/env_0/Scene/Palm"):
    """
    Move the PingTi robot to orbit around the palm crown centroid at the given
    clockwise yaw angle (degrees from default position).

    PingTi forward axis is -Y at yaw=0, so the robot faces the tree when:
        yaw = arctan2(face_x, -face_y)  where face = centroid - robot_xy
    """
    import math
    import torch
    import omni.usd
    ...
    # Default robot position (yaw=0 reference)
    default_x = 0.30706
    default_y = 0.49191
    default_z = 4.65058

    # Crown centroid — the true orbit center accounting for trunk slant
    crown_cx, crown_cy = get_crown_centroid(stage, palm_prim_path)
    print(f"[INFO] Crown centroid: ({crown_cx:.4f}, {crown_cy:.4f})")

    # Radius = distance from default position to crown centroid
    dx0 = default_x - crown_cx
    dy0 = default_y - crown_cy
    radius = math.sqrt(dx0 * dx0 + dy0 * dy0)
    base_angle = math.atan2(dy0, dx0)  # angle of default position in standard coords

    # Clockwise rotation: subtract angle in standard math (CCW-positive) coords
    yaw_rad = math.radians(yaw_deg)
    new_angle = base_angle - yaw_rad

    new_x = crown_cx + radius * math.cos(new_angle)
    new_y = crown_cy + radius * math.sin(new_angle)

    # Yaw so robot faces the crown centroid; PingTi forward = -Y at yaw=0
    face_x = crown_cx - new_x
    face_y = crown_cy - new_y
    facing_yaw = math.atan2(face_x, -face_y)

    quat = _yaw_to_quat_wxyz(facing_yaw)
    print(f"[INFO] Repositioning robot to ({new_x:.4f}, {new_y:.4f}, {default_z:.4f}), yaw={math.degrees(facing_yaw):.1f}°")

    robot = env.scene["robot"]
    pos = torch.tensor([[new_x, new_y, default_z]], device=env.device)
        ...
        if args_cli.use_lerobot_recorder:
            ...
            if args_cli.resume:
                ...
                if os.path.exists(args_cli.dataset_file):
                    print(f"[INFO] Overwriting existing dataset file: {args_cli.dataset_file}")
